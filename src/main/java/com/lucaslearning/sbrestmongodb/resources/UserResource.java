...
public class UserResource {
	...
	public ResponseEntity<Void> delete(@PathVariable String id){
		service.delete(id);
		return ResponseEntity.noContent().build(); //retorna uma resposta sem nada (codigo 204)
	}
	
	@RequestMapping(value ="/{id}",	method = RequestMethod.PUT)
	public ResponseEntity<Void> update(@RequestBody UserDTO objDto, @PathVariable String id){
		User obj = service.fromDTO(objDto); //converter dto par user
		obj.setId(id); //garantir que o objeto tenha o id enviado na requisicao
